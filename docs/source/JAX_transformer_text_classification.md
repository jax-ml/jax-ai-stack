...
# Text classification with a transformer  with JAX for AI
...
This tutorial demonstrates how to build a text classifier using a transformer language model using JAX, [Flax NNX](http://flax.readthedocs.io) and [Optax](http://optax.readthedocs.io). We will learn how to perform sentiment analysis on movie reviews from the [IMDB dataset](https://www.tensorflow.org/api_docs/python/tf/keras/datasets/imdb), classifying them as positive or negative.

Here, you will learn how to:

- Load and preprocess the dataset.
- Define the transformer model with Flax and JAX
- Create the loss and training step-related functions
- Train the model
...
This section details the loading, preprocessing, and preparation of the [IMDB dataset](https://ai.stanford.edu/~amaas/data/sentiment/) movie review dataset. It is available on [TF Datasets](https://www.tensorflow.org/api_docs/python/tf/keras/datasets/imdb) and has 25,000 movies reviews from IMDB, labeled by sentiment (positive/negative). The reviews are encoded as a list of word indices, where words are indexed by frequency. Positive and negative sentiments have integer labels `1` or `0`, respectively.

Let's create a function that loads and prepares the dataset (`prepare_imdb_dataset()`), and another function for padding the (NumPy) array sequences to a fixed length (`pad_sequences()`):
...
def prepare_imdb_dataset(num_words: int, index_from: int, oov_char: int = 2) -> tuple:
    ...
    # We'll have training and test sets.
...
## Making data loading and preprocessing more efficient

We will utilize the Grain library for efficient data loading and preprocessing. Grain supports custom setups where data sources may come in different forms, and we will implement the `grain.RandomAccessDataSource` (a Grain data source) interface. Check out [PyGrain Data Sources](https://github.com/google/grain/blob/main/docs/source/data_sources.md) for more details.

Our dataset is comprised of relatively small NumPy arrays, so our `grain.RandomAccessDataSource` is not complicated:

```{code-cell} ipython3
# Implement a custom data source for Grain, handling the IMDB dataset.
...
**Note:** We will not use [device parallelism](https://jax.readthedocs.io/en/latest/notebooks/Distributed_arrays_and_automatic_parallelization) or sharding for [Single-Program Multi-Data](https://en.wikipedia.org/wiki/Single_program,_multiple_data), since this is a single-device setup. During sharding, the training data is run via multiple parts - batches - in parallel and simultaneously across different devices, such as GPUs and Google TPUs. Larger batch sizes can speed up training. You can learn more about it in the [miniGPT with JAX AI tutorial](https://jax-ai-stack.readthedocs.io/en/latest/JAX_for_LLM_pretraining.html) and JAX documentation's [tensor parallelism](https://jax.readthedocs.io/en/latest/notebooks/Distributed_arrays_and_automatic_parallelization) page.
...
train_loader = grain.DataLoader(
    ...
    sampler=train_sampler,  # A sampler determining how to access the data.
    worker_count=4,         # Number of child processes launched to parallelize the transformations.
    worker_buffer_size=2,   # Number count of output batches to produce in advance per worker.
    operations=[
...
test_loader = grain.DataLoader(
    ...
    sampler=test_sampler,  # A sampler to determine how to access the data.
    worker_count=4,        # Number of child processes launched to parallelize the transformations.
    worker_buffer_size=2,  # Number count of output batches to produce in advance per worker.
    operations=[
...
Here we construct the transformer model using JAX and Flax NNX.
...
class TransformerBlock(nnx.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, ff_dim: int, rngs: nnx.Rngs, rate: float = 0.1):
        ...
        # First linear transformation for the feed-forward network with `flax.nnx.Linear`.
        ...
        # First dropout with `flax.nnx.Dropout`.
        self.dropout_1 = nnx.Dropout(rate, rngs=rngs)
        # Second dropout with `flax.nnx.Dropout`.
...
Next, let's create a class that transforms tokens and positions into embeddings that will be fed into the transformer (via `TransformerBlock`)

Our `TokenAndPositionEmbedding()` class will combine token embeddings (words in an input sentence) with positional embeddings (the position of each word in a sentence). (It handles embedding both word tokens and their positions within the sequence.)
...
This section details the construction of our the transformer model, which includes `TokenAndPositionEmbedding` for transforming tokens and positions into embeddings, transformer blocks (`TransformerBlock`) for processing the embedded sequences, and other layers.

Create a new class for the entire transformer model, subclassing `flax.nnx.Module`:
...
class MyModel(nnx.Module):
    def __init__(self, rngs: nnx.Rngs):
        """ Initializes the transformer block with the `TokenAndPositionEmbedding`, `TransformerBlock`,
        and other layers.
...
To train our model, construct an `flax.nnx.Optimizer` object with our `MyModel` transformer and the Adam optimizer.
...
Define the loss function - `compute_losses_and_logits()` - using `optax.softmax_cross_entropy_with_integer_labels`:
...
Set the evaluation metrics using `flax.nnx.MultiMetric`.
...
Set up a function for training one epoch (`train_one_epoch`) and computing metrics on the training and test sets:
...
Plot the accuracy and loss metrics on the test set:
...
The model achievs around 85% accuracy after just 10 epochs. Based on the loss plot, during the last few epochs the loss value didn't improve much, which may indicate that our model converged.
