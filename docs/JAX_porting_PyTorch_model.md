...
Let us say we have a trained PyTorch computer-vision model to classify images that we would like to port to JAX. We will use [`TorchVision`](https://pytorch.org/vision/stable/index.html) to provide [MaxVit](https://pytorch.org/vision/stable/models/maxvit.html) model trained on ImageNet (MaxViT: Multi-Axis Vision Transformer, https://arxiv.org/abs/2204.01697).
...
MAxVit model is implemented in TorchVision [here](https://pytorch.org/vision/stable/_modules/torchvision/models/maxvit.html#maxvit_t). If we inspect the forward pass of the model, we can see that it contains three high-level parts:
- stem: few convolutions, batchnorms, GELU activations.
...
We can download an image of a Pembroke Corgy dog from TorchVision's gallery ([url](https://github.com/pytorch/vision/blob/main/gallery/assets/dog1.jpg?raw=true)) together with ImageNet classes dictionary ([url](https://raw.githubusercontent.com/pytorch/vision/refs/heads/main/gallery/assets/imagenet_class_index.json)):
...
Let us now define all the necessary model parts in JAX using Flax module. PyTorch model's implementation can be found [here](https://pytorch.org/vision/stable/_modules/torchvision/models/maxvit.html#maxvit_t), we can now list the modules and helper functions we implement in JAX:
...
Flax NNX module is very similar to PyTorch `torch.nn` module and we can simply map the following modules between PyTorch and Flax:
...
If PyTorch model defines a learnable parameter and a buffer:
```python
...
```
equivalent code in Flax would be
```python
...
Please note the some tiny differences between `torch.nn` and Flax when porting models:
- We should pass `rngs` to all NNX modules with parameters: e.g. `nnx.Linear(..., rngs=nnx.Rngs(0))`
- Convolution 2D in Flax:
  - we need explicitly define `kernel_size`, `strides` as two ints tuples, e.g. `(3, 3)`
  - if `padding` should be defined as integer, e.g. 2, it should be defined as `((2, 2), (2, 2))`
- Batch Normalization: momentum in `torch.nn` is defined as `1.0 - momentum` in Flax
- 4D input array in Flax should be in NHWC format, i.e. of shape (N, H, W, C) compared to NCHW format (or (N, C, H, W) shape) in PyTorch.

+++

Below we implement one by one all the modules from the above list and add a simple forward pass check.
...
PyTorch source implementation: https://pytorch.org/vision/stable/_modules/torchvision/ops/misc.html#Conv2dNormActivation
...
PyTorch source implementation: https://pytorch.org/vision/stable/_modules/torchvision/ops/misc.html#SqueezeExcitation
...
PyTorch source implementation: https://pytorch.org/vision/stable/_modules/torchvision/ops/stochastic_depth.html#StochasticDepth
...
PyTorch source implementation: https://pytorch.org/vision/stable/_modules/torchvision/models/maxvit.html#MBConv
...
PyTorch source implementation: https://pytorch.org/vision/stable/_modules/torchvision/models/maxvit.html#RelativePositionalMultiHeadAttention
...
PyTorch source implementation: https://pytorch.org/vision/stable/_modules/torchvision/models/maxvit.html
...
Finally, we can assemble evreything together and define MaxVit model.
PyTorch source implementation: https://pytorch.org/vision/stable/_modules/torchvision/models/maxvit.html
    ...
    def _init_weights(self, rngs):
        ...
        for name, module in self.iter_modules():
            if isinstance(module, (nnx.Conv, nnx.Linear)):
                module.kernel.value = normal_initializer(rngs(), module.kernel.value.shape, module.kernel.value.dtype)
                if module.bias.value is not None:
                    module.bias.value = jnp.zeros(module.bias.value.shape, dtype=module.bias.value.dtype)
...
Below we define a helper class `Torch2Flax` to copy parameters and buffers from a torch module into equivalent Flax module.
...
class Torch2Flax:
    ...
    modules_mapping_info = {
        ...
        torch_mod: {
            ...
            (nn.AvgPool2d, type(lambda x: nnx.avg_pool(x))),
            (nn.AdaptiveAvgPool2d, type(lambda x: nnx.avg_pool(x))),
        ]
    }
...
Let us now test our JAX modules. We only test the result of the forward pass in the inference mode such that we avoid disrcrepancies related to random layers like Dropout, StochasticDepth, etc.
By default, we use absolute error tolerence `1e-3` when comparing the JAX output vs expected PyTorch result.
For larger modules we set the device to CPU for JAX model to execute on in order to reduce the errors between CPU and CUDA.
...
Finally, we can check MaxVit implementation. Note that we raised absolute tolerence to `1e-1` when compare JAX output logits vs PyTorch expected logits.
